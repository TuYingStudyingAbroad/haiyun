...
#import "fansObject.h"
#import "noDataView.h"
@interface FSdefsViewController ()<fansSortViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation FSdefsViewController
...
-(void)loadNewData:(BOOL)isNew{
    ...
    [MKNetworking MKSeniorGetApi:@"/fans/list" paramters:param completion:^(MKHttpResponse *response)
     {
         [ hud hide:YES];
         NSLog(@"fansDic%@",response.responseDictionary);
         // 拿到当前的下拉刷新控件，结束刷新状态
...
-(void)didPressBtnTag:(NSInteger)btnTag withUPDown:(NSInteger)ud{
    NSLog(@"%ld     %ld",(long)btnTag, (long) ud);
    if (ud==1) {
        self.upDown=1;
    }else{
        self.upDown=2;
    ...
    NSLog(@"%ld     %ld",(long)btnTag, (long) self.upDown);
