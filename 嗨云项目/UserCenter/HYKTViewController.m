//
//  HYKTViewController.m
//  嗨云项目
//
//  Created by 唯我独尊 on 16/11/22.
...
@interface HYKTViewController ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate,MKMarketingCellDelegate>
...
@implementation HYKTViewController
...
- (void)loadDataNewly:(BOOL)isNew
{
    ...
    [MKNetworking MKSeniorGetApi: shouyeApi paramters:paramterDic completion:^(MKHttpResponse *response) {
        ...
        path = [path stringByAppendingPathComponent:@"999999999"];
...
{
    if ( ISNSStringValid(imageUrl) ) {
        [UIAlertView showWithTitle:@"提示" message:@"是否保存图片？" cancelButtonTitle:@"取消" otherButtonTitles:@[@"保存"] tapBlock:^(UIAlertView * _Nonnull alertView, NSInteger buttonIndex) {
            if ( buttonIndex ) {
                ...
                }else
                {
                    ...
                    [manager downloadImageWithURL:url options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                        ...
                        [MBProgressHUD showMessageIsWait:@"图片已保存至手机相册" wait:YES];
                    }];
                }
            }
