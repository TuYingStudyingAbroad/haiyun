//
//  firstPopularizeViewController.m
//  嗨云项目
//
//  Created by 小辉 on 16/11/22.
//  Copyright © 2016年 haiyun. All rights reserved.
//

#import "firstPopularizeViewController.h"
...
@interface firstPopularizeViewController ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate,MKMarketingCellDelegate>
...
@implementation firstPopularizeViewController
...
//    rightButtonView.backgroundColor=[UIColor redColor];
    [rightButtonView addSubview:shareBtn];
    [shareBtn setImage:[UIImage imageNamed:@"fenxiang"] forState:UIControlStateNormal];
    [shareBtn addTarget:self action:@selector(shareNoViewShow) forControlEvents:UIControlEventTouchUpInside];
...
-(void)shareNoViewShow
{
    ...
    {
        _activitySharView = [[HYShareActivityView alloc] initWithButtons:@[@(HYSharePlatformTypeWechatSession),@(HYSharePlatformTypeWechatTimeline)]shareTypeBlock:^(HYSharePlatformType type)
                             {
...
- (void)shareMoreActionClickType:(NSInteger)types index:(NSInteger)index
{
    ...
    info.title = [MKFlagShared sharedInstance].tittleName;
    if ( [getUserCenter isLogined] ) {
        info.url = [NSString stringWithFormat:@"%@/main.html?pagetype=%ld&pageid=%@&share_user_id=%ld",BaseHtmlURL, (long)self.pageType,self.pageId,(long)getUserCenter.userInfo.userId];
    }else{
        info.url = [NSString stringWithFormat:@"%@/main.html?pagetype=%ld&pageid=%@",BaseHtmlURL, (long)self.pageType,self.pageId];
...
- (void)loadDataNewly:(BOOL)isNew
{
    ...
    NSDictionary * paramterDic=nil;
        ...
        paramterDic=@{@"page_id":self.pageId};
    ...
    [MKNetworking MKSeniorGetApi: shouyeApi paramters:paramterDic completion:^(MKHttpResponse *response) {
        ...
        path = [path stringByAppendingPathComponent:self.pageId];
...
- (void)loadFromDictionary:(NSDictionary *)data
{
    ...
    //
    if ([data HYValueForKey:@"pagename"]) {
        [MKFlagShared sharedInstance].tittleName=[data HYValueForKey:@"pagename"];
        self.title=[MKFlagShared sharedInstance].tittleName;
    }
