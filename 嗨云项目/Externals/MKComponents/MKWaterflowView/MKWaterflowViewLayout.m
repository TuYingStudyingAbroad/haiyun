...
- (MKWaterflowViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    NSMutableArray *itemsFrame = self.sectionFrames[[NSString stringWithFormat:@"%ld",(long)indexPath.section]];
...
- (void)prepareForWaterflowViewUpdates:(NSArray *)updateItems{
    ...
    for (NSIndexPath *indexPath in updateItems){
        ...
        self.layoutStacksOfSection[[NSString stringWithFormat:@"%ld",(long)col]] = NSStringFromCGSize(CGSizeMake(itemSize.width, oringY+itemSize.height));
        //build the map from indexPath to frame
        self.layoutFramesOfSection[[NSString stringWithFormat:@"%ld",(long)indexPath.row]] = NSStringFromCGRect(itemFrame);
...
- (void)layoutItems:(NSInteger)numberOfItems InSection:(NSInteger)section{
    ...
    self.currentContentSize = contentSize;
    [self.sectionFrames setObject:itemsFrame forKey:[NSString stringWithFormat:@"%ld",(long)section]];
}
