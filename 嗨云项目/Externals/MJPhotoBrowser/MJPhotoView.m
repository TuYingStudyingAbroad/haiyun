...
+ (UIView *)maskView {
    static UIView *_staticMaskView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _staticMaskView = [[UIView alloc]init];
        _staticMaskView.backgroundColor = [UIColor colorWithHex:0xD8000000];
    });
    return _staticMaskView;
}
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        __block MBProgressHUD *loadingHud = [MBProgressHUD showHUDAddedTo:self animated:YES];
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        [MJPhotoView maskView].alpha = 0;
        [UIView animateWithDuration:0.3 animations:^{
            _imageView.frame = imageFrame;
            [MJPhotoView maskView].alpha = 1.0;
...
- (void)layoutSubviews {
    ...
    if (disableLayoutSubviews) { return; }
...
//    }
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        [[MJPhotoView maskView] removeFromSuperview];
    }];
}
