...
@implementation HYContactsObject

- (void)setName:(NSString *)name
{
    if (name) {
        _name=name;
        _initial=[_name.initial lowercaseString];
    }
}
...
@implementation HYContactDataHelper
...
+ (NSMutableArray *) getFriendListDataBy:(NSMutableArray *)array
{
    ...
    NSArray *serializeArray = [(NSArray *)array sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {//排序
        int i;
        NSString *strA = ((HYContactsObject *)obj1).initial;
        NSString *strB = ((HYContactsObject *)obj2).initial;
    ...
    for (HYContactsObject *user in serializeArray) {
        char c = [user.initial characterAtIndex:0];
        if (!isalpha(c)) {
            [oth addObject:user];
        }
        else if (c != lastC){
...
+ (NSMutableArray *)getFriendListSectionBy:(NSMutableArray *)array
{
    ...
    for (NSArray *item in array) {
        HYContactsObject *user = [item objectAtIndex:0];
        char c = [user.initial characterAtIndex:0];
        if (!isalpha(c)) {
            c = '#';
        }
